...
func (r *ReflectClientBuilder) InvokeRPC(methodName string) {
	if r.ctx == nil {
	...
	err = reflect.InvokeRPC(r.ctx, descSource, r.cc, methodName, append(r.addlHeaders, r.rpcHeaders...), h, rf.Next)
	if err != nil {
		log.GetLogger(r.ctx).Error(errors.Wrapf(err, "Error invoking method %q", methodName))
	}
	fmt.Printf("Sent: %d and received %d\n", rf.NumRequests(), h.NumResponses)
